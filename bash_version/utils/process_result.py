...
# 兼容 vllm 和 sglang 后端的字段名差异
#   vllm:   total_token_throughput, model_id
#   sglang: total_throughput,       无 model_id
total_token_throughput = float(bmk_result.get('total_token_throughput', bmk_result.get('total_throughput', 0)))

data = {
    'hw': hw,
    'conc': int(bmk_result.get('max_concurrency', bmk_result.get('concurrency', 0))),
    'image': image,
    'model': bmk_result.get('model_id', model_prefix),
...
if is_multinode:
    ...
    multi_node_data = {
        ...
        'tput_per_gpu': total_token_throughput / (prefill_gpus + decode_gpus),
        'output_tput_per_gpu': float(bmk_result['output_throughput']) / decode_gpus,
        'input_tput_per_gpu': (total_token_throughput - float(bmk_result['output_throughput'])) / prefill_gpus,
    }
    ...
    single_node_data = {
        ...
        'input_tput_per_gpu': (total_token_throughput - float(bmk_result['output_throughput'])) / tp_size,
    }
